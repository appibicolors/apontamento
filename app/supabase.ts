...
export async function loadOrders(token:string):Promise<Ordem[]>{const response=await supabaseFetch("/rest/v1/ordens_producao?select=id,numero_op,cliente,nf_entrada,data_op,codigo_artigo,artigo,pecas,metros,peso,status,criado_em,encerrada_em,encerrada_por,motivo_encerramento,operacoes(id,codigo,descricao,status,sequencia,apontamentos(id,operador_id,maquina_id,inicio_em,termino_em,observacao,motivo_finalizacao,grupo_id,duracao_rateada_segundos,quantidade_rateio,criterio_rateio))&order=criado_em.desc",{},token);return response.json()}
