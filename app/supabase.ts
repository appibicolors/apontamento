...
export type Apontamento={id:string;operador_id:string;maquina_id:string;inicio_em:string;termino_em:string|null;observacao:string|null;motivo_finalizacao?:string|null;grupo_id?:string|null;duracao_rateada_segundos?:number|null;quantidade_rateio?:number|null;criterio_rateio?:string|null};
export type Operacao={id:string;codigo:string;descricao:string;status:string;sequencia:number;apontamentos?:Apontamento[]};
export type Ordem = { id:string; numero_op:string; cliente:string; nf_entrada?:string|null; data_op?:string|null; prazo_dias?:number|null; prazo_estimado?:string|null; codigo_artigo?:string|null; artigo:string; pecas?:number; metros?:number; peso?:number; criado_em?:string|null;encerrada_em?:string|null;encerrada_por?:string|null;motivo_encerramento?:string|null;status:"rascunho"|"aguardando"|"em_producao"|"finalizada"|"cancelada"; operacoes?:Operacao[] };
...
export async function loadOrders(token:string):Promise<Ordem[]>{
  const operations="operacoes(id,codigo,descricao,status,sequencia,apontamentos(id,operador_id,maquina_id,inicio_em,termino_em,observacao,motivo_finalizacao,grupo_id,duracao_rateada_segundos,quantidade_rateio,criterio_rateio))";
  const common=`id,numero_op,cliente,nf_entrada,data_op,codigo_artigo,artigo,pecas,metros,peso,status,criado_em,encerrada_em,encerrada_por,motivo_encerramento,${operations}`;
  try{const response=await supabaseFetch(`/rest/v1/ordens_producao?select=id,numero_op,cliente,nf_entrada,data_op,prazo_dias,prazo_estimado,codigo_artigo,artigo,pecas,metros,peso,status,criado_em,encerrada_em,encerrada_por,motivo_encerramento,${operations}&order=criado_em.desc`,{},token);return response.json()}
  catch(error){if(!(error instanceof Error)||!/prazo_dias|prazo_estimado/i.test(error.message))throw error;const response=await supabaseFetch(`/rest/v1/ordens_producao?select=${common}&order=criado_em.desc`,{},token);const orders=await response.json() as Ordem[];return orders.map(order=>({...order,prazo_dias:null,prazo_estimado:null}))}
}
...
export async function startOperation(token:string,operacaoId:string,operadorId:string,maquinaId:string,observacao:string){await supabaseFetch("/rest/v1/rpc/iniciar_execucao",{method:"POST",body:JSON.stringify({p_operacao_id:operacaoId,p_operador_id:operadorId,p_maquina_id:maquinaId,p_observacao:observacao||null})},token)}
export async function finishOperation(token:string,operacaoId:string,observacao:string,motivo="normal"){await supabaseFetch("/rest/v1/rpc/finalizar_execucao",{method:"POST",body:JSON.stringify({p_operacao_id:operacaoId,p_observacao:observacao||null,p_motivo:motivo})},token)}
...
export async function startGroupedOperation(token:string,data:{code:string;operatorId:string;machineId:string;criterion:string;items:Array<{operacao_id:string;quantidade:number}>;note:string}){const response=await supabaseFetch("/rest/v1/rpc/iniciar_apontamento_agrupado_v2",{method:"POST",body:JSON.stringify({p_codigo_operacao:data.code,p_operador_id:data.operatorId,p_maquina_id:data.machineId,p_criterio:data.criterion,p_itens:data.items,p_observacao:data.note||null})},token);return response.json() as Promise<string>}
export async function finishGroupedOperation(token:string,groupId:string,note:string,motivo="normal"){await supabaseFetch("/rest/v1/rpc/finalizar_apontamento_agrupado_v2",{method:"POST",body:JSON.stringify({p_grupo_id:groupId,p_observacao:note||null,p_motivo:motivo})},token)}
export async function resumeGroupedOperation(token:string,groupId:string){const response=await supabaseFetch("/rest/v1/rpc/retomar_apontamento_agrupado",{method:"POST",body:JSON.stringify({p_grupo_origem_id:groupId})},token);return response.json() as Promise<string>}
export async function addOrdersToGroupedOperation(token:string,groupId:string,items:Array<{operacao_id:string;quantidade:number}>,reason:string){await supabaseFetch("/rest/v1/rpc/adicionar_ops_apontamento_agrupado",{method:"POST",body:JSON.stringify({p_grupo_id:groupId,p_itens:items,p_motivo:reason})},token)}
...
export const adminUpdateOrder=(token:string,data:{id:string;numero:string;cliente:string;artigo:string;prazoDias:number|null})=>adminRpc(token,"admin_editar_op_v2",{p_id:data.id,p_numero_op:data.numero,p_cliente:data.cliente,p_artigo:data.artigo,p_prazo_dias:data.prazoDias});
export const adminDeleteOrder=(token:string,id:string)=>adminRpc(token,"admin_excluir_op",{p_id:id});
export const adminSetOrderClosed=(token:string,id:string,closed:boolean,reason:string)=>adminRpc(token,"admin_alterar_encerramento_op",{p_id:id,p_encerrar:closed,p_motivo:reason});
...
export const adminUpdateUser=(token:string,id:string,name:string,profile:"admin"|"operador"|"consulta")=>adminRpc(token,"admin_editar_usuario",{p_id:id,p_nome:name,p_perfil:profile});
