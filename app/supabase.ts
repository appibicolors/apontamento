...
export type Ordem = { id:string; numero_op:string; cliente:string; nf_entrada?:string|null; data_op?:string|null; artigo:string; pecas?:number; metros?:number; peso?:number; status:"rascunho"|"aguardando"|"em_producao"|"finalizada"|"cancelada"; operacoes?:Operacao[] };
...
export async function loadOrders(token:string):Promise<Ordem[]>{const response=await supabaseFetch("/rest/v1/ordens_producao?select=id,numero_op,cliente,nf_entrada,data_op,artigo,pecas,metros,peso,status,operacoes(id,codigo,descricao,status,sequencia,apontamentos(id,operador_id,maquina_id,inicio_em,termino_em,observacao))&order=criado_em.desc",{},token);return response.json()}
