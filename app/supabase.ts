...
export type Ordem = { id:string; numero_op:string; cliente:string; nf_entrada?:string|null; data_op?:string|null; codigo_artigo?:string|null; artigo:string; pecas?:number; metros?:number; peso?:number; encerrada_em?:string|null;encerrada_por?:string|null;motivo_encerramento?:string|null;status:"rascunho"|"aguardando"|"em_producao"|"finalizada"|"cancelada"; operacoes?:Operacao[] };
...
export async function loadOrders(token:string):Promise<Ordem[]>{const response=await supabaseFetch("/rest/v1/ordens_producao?select=id,numero_op,cliente,nf_entrada,data_op,codigo_artigo,artigo,pecas,metros,peso,status,encerrada_em,encerrada_por,motivo_encerramento,operacoes(id,codigo,descricao,status,sequencia,apontamentos(id,operador_id,maquina_id,inicio_em,termino_em,observacao,motivo_finalizacao,grupo_id,duracao_rateada_segundos,quantidade_rateio,criterio_rateio))&order=criado_em.desc",{},token);return response.json()}
