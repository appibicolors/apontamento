...
export default function Home() {
  ...
  const displayedOrders=liveOrders.length?liveOrders.map(op=>{const operations=[...(op.operacoes??[])].sort((a,b)=>a.sequencia-b.sequencia);const covered=operations.filter(x=>x.apontamentos?.some(point=>point.termino_em)).length;const current=operations.find(x=>x.status==="em_andamento")??operations.find(x=>!x.apontamentos?.some(point=>point.termino_em))??operations.at(-1);const progress=operations.length?Math.round(covered/operations.length*100):0;return{numero:op.numero_op,artigo:op.artigo,cliente:op.cliente,progresso:progress,status:op.encerrada_em?"Encerrada":op.status==="em_producao"?"Em produção":"Aguardando",etapa:current?.descricao??"Fluxo não cadastrado",maquina:"—",operador:"—",inicio:"—",tom:op.encerrada_em?"green":op.status==="em_producao"?"blue":"amber"};}):ordens;
