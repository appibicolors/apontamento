...
export default function Home() {
  ...
  async function fileSelected(file?:File){if(!file)return;setFileName(file.name);setImporting(true);setModal("import");setParsedOrders([]);try{const parsed=await parseProductionOrders(file);setParsedOrders(parsed);if(!parsed.length)setNotice("Nenhuma ordem de produção foi encontrada nesse PDF.")}catch{setNotice("Não foi possível ler esse PDF. Verifique o arquivo e tente novamente.")}finally{setImporting(false);if(inputRef.current)inputRef.current.value=""}}
  ...
  async function publishOrders(){if(!parsedOrders.length)return;setImporting(true);try{await saveOrders(parsedOrders,session);setLiveOrders(await loadOrders(session.access_token));setModal(null);setNotice(`${parsedOrders.length} OP${parsedOrders.length===1?"":"s"} gravada${parsedOrders.length===1?"":"s"} no Supabase com sucesso.`)}catch(error){setNotice(error instanceof Error?`Erro ao gravar: ${error.message}`:"Não foi possível gravar as OPs.")}finally{setImporting(false)}}
