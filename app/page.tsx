...
export default function Home() {
  ...
  const [modal, setModal] = useState<"import" | "pointing" | "machines" | "operators" | "history" | "admin" | "management" | null>(null);
  const [pointingGrouped,setPointingGrouped]=useState(false);
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav aria-label="Navegação principal">
          ...
          <a href="#apontamento" onClick={event=>{event.preventDefault();setPointingGrouped(false);setModal("pointing")}}><span>◎</span>Apontamento</a>
          ...
          {isAdmin&&<a href="#gestao" onClick={event=>{event.preventDefault();setModal("management")}}><span>▥</span>Gestão de máquinas</a>}
      ...
      {modal && <div className="modal-backdrop" role="presentation" onMouseDown={() => setModal(null)}>{modal==="management"&&isAdmin?<div role="dialog" aria-modal="true" aria-label="Painel gerencial de máquinas" onMouseDown={e=>e.stopPropagation()}><MachineDashboard orders={liveOrders} profiles={profiles} machines={machines} onClose={()=>setModal(null)} onRefresh={async()=>setLiveOrders(await loadOrders(session.access_token))}/></div>:modal==="admin"&&isAdmin?<div role="dialog" aria-modal="true" aria-label="Administração" onMouseDown={e=>e.stopPropagation()}><AdminScreen orders={liveOrders} profiles={profiles} machines={machines} token={session.access_token} onClose={()=>setModal(null)} onRefresh={async()=>{const [orders,people,equipment]=await Promise.all([loadOrders(session.access_token),loadProfiles(session.access_token),loadMachines(session.access_token)]);setLiveOrders(orders);setProfiles(people);setMachines(equipment)}}/></div>:modal==="history"?<div role="dialog" onMouseDown={e=>e.stopPropagation()}><HistoryScreen orders={liveOrders} profiles={profiles} machines={machines} token={session.access_token} onClose={()=>setModal(null)} onResume={async()=>{setLiveOrders(await loadOrders(session.access_token));setPointingGrouped(true);setModal("pointing")}}/></div>:modal==="pointing"?<div role="dialog" aria-modal="true" aria-label="Apontamento de produção" onMouseDown={e=>e.stopPropagation()}><PointingScreen orders={liveOrders} profiles={profiles} machines={machines} token={session.access_token} userId={session.user.id} initialGrouped={pointingGrouped} onClose={()=>setModal(null)} onRefresh={async()=>setLiveOrders(await loadOrders(session.access_token))}/></div>:modal==="machines"||modal==="operators"?<div role="dialog" aria-modal="true" onMouseDown={e=>e.stopPropagation()}><ManagementScreen kind={modal} machines={machines} profiles={profiles} token={session.access_token} onClose={()=>setModal(null)} onRefresh={async()=>{const [people,equipment]=await Promise.all([loadProfiles(session.access_token),loadMachines(session.access_token)]);setProfiles(people);setMachines(equipment)}}/></div>:<section className="modal" role="dialog" aria-modal="true" aria-label="Importar ordem de produção" onMouseDown={(e) => e.stopPropagation()}>
